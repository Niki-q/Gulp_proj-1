...
$btn_switchable.on({
  click: e => {
    ...
    for (element in list_of_path_for_icons) {
      if ($(e.target).hasClass(element)) category_class = element;
    }

    if ($(e.target).hasClass('button_activated')) changeScrOfIconBySprite($svg, list_of_path_for_icons[category_class].activate);else changeScrOfIconBySprite($svg, list_of_path_for_icons[category_class].inactive);
  },
  activate: e => {
    console.log(1);
  }
...
const $tf_ordinary = $('.input.text-field-ordinary__input'); // Getter
...
function removeClean(nodes) {
  ...
  $(nodes.$parent).addClass(inactiveclass); // $(nodes.$parent).addClass(promoclass)
...
const $tf_small = $('.input.text-field-small__input'); // Listener
